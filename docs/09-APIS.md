...
|---|---|---|
| GET `/vacantes` | Las vacantes publicadas **de todas las empresas activas**, cada una con el nombre de la suya. Las de una empresa suspendida no salen. Cada una trae su `remuneracion`. **Desde el 25/09 (`V62`) trae además `publicadaEn`** —puede venir vacía en una vieja— **y `ciudad`**: `{codigo, nombre, departamento}` del catálogo, o nula si la vacante no la tiene. `ubicacion` sigue viajando y ahora es la zona o referencia | Cualquiera, sin token |
| GET `/vacantes/{id}` | El detalle público, con los requisitos indispensables, la `remuneracion`, `publicadaEn` y `ciudad` | Cualquiera |
...
**La vacante también tiene ciudad, y sale del mismo catálogo** (25/09/2026, `V62`). Es lo que
deja al portal filtrar por ciudad: «Lima» y «LIMA» escritas a mano eran dos filtros. `ciudad`
es una provincia o «Fuera del Perú» (`EXT`, con el departamento vacío). **Buscar, filtrar y
ordenar lo hace el portal**, en el navegador y sobre la lista entera: `GET /vacantes` no tiene
...
**La ciudad de la vacante** (25/09/2026, `V62`). El alta (`POST`) y la edición (`PUT`) aceptan
`ciudadUbigeo`: una provincia del catálogo o `EXT`. Un código que el catálogo no ofrece —un
departamento, un distrito, uno inventado— responde **400 «Esa ciudad no está en el catálogo»** y
no guarda nada. Sin ciudad se acepta: **exigirla es cosa del formulario del panel, no de la
API**, igual que la modalidad. Al editar, el código que ya estaba guardado se acepta aunque el
catálogo lo haya retirado después, para que corregir el horario no obligue a cambiar la ciudad.
Si la vacante está publicada, cambiar la ciudad avisa como cualquier dato que ve el candidato
(«Ciudad: — → Arequipa»), y el texto libre de siempre, `ubicacion`, se nombra en el aviso como
**«Zona o referencia»**. En la auditoría los campos siguen llamándose `ciudadUbigeo` y
`ubicacion`.
