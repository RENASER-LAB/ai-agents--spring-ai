...
| | |
|---|---|
| Base de datos | **Flyway es el dueño del esquema**: V1-V51 en `src/main/resources/db/migration` (la V49 y la V50 solo siembran pesos; la V51 trae `lectura_cv_perfil` y las columnas de foto, portada, currículum y diploma del perfil). `ddl-auto: validate`. El recuento de tablas está en [Modelo de datos](05-MODELO-DE-DATOS.md) |
...
| Tests | **1150 en verde (02/09/2026)** con `./mvnw verify`: 1007 unitarios y 143 de integración (6 de esos se saltan: los que gastan saldo o mandan correo de verdad). Desde entonces #62 (03/09, el ranking de la prueba) añadió 5 unitarios y #63-#65 (04/09, el ponderado, el semáforo sin pesos y los pesos de Administración) otros 13, y #66 (06/09, los archivos del perfil) otros 80, sin recontar en una corrida completa: la próxima tiene que dar 1248. Ver [Comprobaciones automáticas](COMPROBACIONES-AUTOMATICAS.md) |
...
| Lo configurable | Plazo de la evaluación (`dias_plazo_evaluacion`, 14), porcentaje de la criba fina (`porcentaje_criba_fina`, 50), meses de conservación del perfil (`meses_conservar_perfil`, 24), tope mensual de IA por empresa (`tope_mensual_ia`), umbrales de los grupos de prioridad (80 y 65, **sin confirmar por Renaser**) |
