...
class ServicioCalificacionImplTest {
    ...
    void unParDelV3NoRevientaNiAlerta() {
        ...
        when(opciones.findById(11L)).thenReturn(Optional.of(
                Opcion.builder().id(11L).puntaje(BigDecimal.valueOf(4)).build()));
        when(opciones.findById(12L)).thenReturn(Optional.of(
                Opcion.builder().id(12L).puntaje(BigDecimal.ZERO).build()));
